...
cdef
int
d_row_num = data.shape[0]
cdef
int
c_row_num = curves.shape[0]
cdef
int
col_num = curves.shape[1]

cdef
int
data_row, curve_row, col
cdef
int
diff_min_idx
cdef
double
diff, diff_new, diff_col

cdef
long[::1]
out_array = np.zeros(d_row_num, dtype=DTYPE)
cdef
double[::1]
ssds = np.zeros(d_row_num, dtype=DTYPEfloat)
cdef
double[:]
data_subset
cdef
