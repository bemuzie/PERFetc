...
DTYPEfloat2 = np.float32
#DTYPEfloat_t
ctypedef np.int_t DTYPE_t
ctypedef np.float32_t DTYPEfloat2_t
...
@cython.cdivision(True)
#sme as data_2 but different looping/
def ssd_int(float [:,::1] data, float[:, ::1] curves):
    cdef int d_row_num = data.shape[0]
    cdef int c_row_num = curves.shape[0]
    cdef int col_num = curves.shape[1]
    cdef int data_row, curve_row, col
    cdef int diff_min_idx
    cdef float diff, diff_new, diff_col
    cdef long[::1] out_array = np.zeros(d_row_num, dtype=DTYPE)
    cdef float[::1] ssds = np.zeros(d_row_num, dtype=DTYPEfloat2)
    cdef float[:] data_subset
    cdef float[:] curve_subset
